...
export const createProject = async (req, res) =>{
    try{
        ...
        const {workspaceId, description, name, status, start_date, end_date,
            ...
            if(!workspace){
                return res.status(404).json({ meassage: "workspace not found"});
            }
            if(!workspace.members.some((member)=>member.userId === userId && member.role === "ADMIN")){
                return res.status(404).json({ meassage: "you don't have permission to create projects in this workspace"});
...
export const updateProject = async (req, res) =>{
    try{
        ...
        if(!workspace){
            return res.status(404).json({ message: "Worspace not found" });
        }
        if(!workspace.members.some((member)=>member.userId === userId && member.role === "ADMIN")){
...
export const addMember = async(req, res) =>{
    try{
        ...
        // check if user is already a number
